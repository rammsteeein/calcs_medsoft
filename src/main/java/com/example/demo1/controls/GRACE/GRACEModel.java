...
public class GRACEModel {
    ...
    public void calc() {
        ...
        if (isEmpty(age.get()) ||
                isEmpty(hr.get()) ||
                isEmpty(sbp.get()) ||
                isEmpty(creatinine.get()) ||
                killip.get() == null) {

            result.set("Введите все обязательные поля для расчёта");
            return;
        }
        ...
        try {
            ...
            result.set(
                    res.toString() +
                            ...
                            "Суммарно: " + totalPoints + " баллов\n" +
                            "Риск: " + interpretation
            );

        } catch (NumberFormatException e) {
            result.set("Проверьте корректность числовых значений");
        } catch (Exception e) {
            result.set("Ошибка расчёта");
        }
    }
}